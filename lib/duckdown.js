...
Duckdown.prototype.tokenise = function(input) {
	...
	for (var charIndex = 0; charIndex <= chunkLength; charIndex ++) {
		...
		var scanAhead = this.longestToken;
			scanAhead = scanAhead > chunkLength - charIndex ?
						chunkLength - charIndex : scanAhead;
		...
		for (; scanAhead > 0; scanAhead--) {
			...
			if (Grammar.tokenMappings[curPhrase]) {
				...
				// If there's anything in the buffer, clear it and
				// push it into the token list
				...
				// Advance scan pointer by the scanahead allowance we
				// had remaining...
...
Duckdown.prototype.parseToken = function(state, input) {
	...
	for (var stateIndex = state.parserStates.length - 1; stateIndex >= 0; stateIndex--) {
		...
		if (Grammar.tokenMappings[state.currentToken] && Grammar.tokenMappings.hasOwnProperty(state.currentToken)) {
			...
			// search our current state listfor this genus state
			if (state.hasParseState(tokenGenus.state) && !tokenGenus.allowSelfNesting) {
				
				// If we're already subscribed to this state, andaren't allowed to self nest,
				...
				// If the current state allows wrapping (ie we can nest something insideit...)
				// _and_ the semanticsmake sense (e.g. we're not inserting a block element inside
				// a text-level element)
				
				// Our grammar may also have some rules about what can precede this node
				// in order for it to bevalid.
				...
				if (weCanWrap() && semanticsAreCorrect(tokenGenus) && 
					...
					// Add thistoken's state to our state stack
					...
					// Add alink back to the parser...
					tmpDuckNode.parser		= state;
					
					// Some nodes needto know that their previous sibling was culled.
				tmpDuckNode.prevSiblingCulled = state.prevNodeCulled;
					...
					// We'renot the root element. Flush the current parse-buffer to the node children.
				...
				if (tokenGenus.semanticLevel === "block" ||
					tokenGenus.semanticLevel === "textblock" ||
						...
						tmpNodePointer.blockParent = true;
							...
							// Make a two-waylink between the original ancestor and the child.
				...
				// What we do instead is add allthe subsequent tokens (until thecurrent state has an exit condition met)
		...
		// Push to parse buffer (if there's anythingin the current token at all!)
			if (state.currentToken && state.currentToken.length) {
				state.parseBuffer.push(state.currentToken);
			}
			
		// Ifwe're at the end of the document, push data into the current node.
			if (state.tokenPosition >= state.tokens.length -1) {
			
				// We are not the root element. Flush the parse buffer into tnecurrent node.
				if (!!state.currentNode) {
					state.currentNode.children.push.apply(state.currentNode.children,state.parseBuffer);
	...
	//Store whether the previous token was whitespace...
		...
		// Return parser states for thistoken as a css-compatible class string.
	...
	Duckdown.prototype.compile = function(input) {
		...
		// We're getting input thislate in the game?
		...
		var compileResult = (function duckpile(input) {
			...
			for (var index = 0; index < inputList.length; index++) {
				...
				if (currentNode instanceof DuckdownNode) {
					...
					// Itmust also have some text, regardless of how deep we need to go to find it.
					if (currentNode.children.length && currentNode.text().length) {
						var stateGenus = Grammar.stateList[currentNode.state];
						
						if (stateGenus && stateGenus.compile && stateGenus.compile instanceofFunction) {
				...
				// to string andadd to the return buffer.
					...
					// But we check to see whether the replacerfunction was defined first.
					...
					// Just pushthis node onto our return buffer
...
if (module) module.exports = Duckdown;